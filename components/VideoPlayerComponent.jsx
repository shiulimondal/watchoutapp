...
const VideoPlayerComponent = ({ videoUrl }) => {
    ...
    useEffect(() => {
        const fetchProgress = async () => {
            try {
                const savedProgress = await readData("video_progress");
                console.log(savedProgress, "saved progress");
                if (savedProgress !== null && typeof savedProgress === "number") {
                    setProgress(savedProgress);

                    const status = await video.current.getStatusAsync();
                    const newPosition = (savedProgress * status.durationMillis) / 100;
                ...
                console.log("Error fetching video progress:", error.message);
            }
        };
